...
# Output Dir
results_dir = 'BankResult'
os.makedirs(results_dir, exist_ok=True)
df = pd.read_csv('Bank/bank.csv', delimiter=';').head(200)  # only the first 200 rows
...
def evaluate_model(model, X, y, method_name):
    ...
    with open(os.path.join(results_dir, f'bank-result-{method_name}.txt'), 'w') as f:
...
with open(os.path.join(results_dir, 'bank-result-knn-best.txt'), 'w') as f:
...
def cluster_evaluation(model, X, y, method_name):
    ...
    with open(os.path.join(results_dir, f'bank-result-cluster-{method_name}.txt'), 'w') as f:
